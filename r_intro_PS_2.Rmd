...
4. Then, using `cbind` function to combined those two vectors" `test` and `mean_list`, and treat the result as `data.frame` and assigned to the object name `mean_list_1`. You can try with below demo.
```{r}
df <- iris 
means_list <- replicate(100, expr = mean(sample( df$Sepal.Length, 10, replace = TRUE)))
?rep  
...
```{r}
means_list_2 <- data.frame(cbind(test,"means_list" = replicate( 200, expr = mean(sample(df$Sepal.Length,30, replace = TRUE)))))
means_list_3 <- data.frame(cbind(test,"means_list" = replicate( 1000, expr = mean(sample(df$Sepal.Length,50, replace = TRUE)))))
means_list_4 <- data.frame(cbind(test,"means_list" = replicate( 3000, expr = mean(sample(df$Sepal.Length,50, replace = TRUE)))))
means_list_5 <- data.frame(cbind(test,"means_list" = replicate( 10000, expr = mean(sample(df$Sepal.Length,50, replace = TRUE)))))
```
...
```{r}
## Mean for mean_list_1
mean(means_series[[1]][[2]])

## Mean for mean_list_2
mean(means_series[[2]][[2]])

## Mean for mean_list_3
mean(means_series[[3]][[2]])

## Mean for mean_list_4
mean(means_series[[4]][[2]])

## Mean for mean_list_5
mean(means_series[[5]][[2]])

```
```{r}
?bind_rows
df_means_combined <- bind_rows(means_list_1,means_list_2,means_list_3,means_list_4,means_list_5)
str(df_means_combined)
